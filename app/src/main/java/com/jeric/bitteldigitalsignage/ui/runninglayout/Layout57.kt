...
class Layout57 @JvmOverloads constructor(
    context: Context,
    ...
    fun setData(data: List<EventFeedModel>) {
    ...
    private class ScrollHandler(mView: Layout57?) : Handler() {
        private val view: WeakReference<Layout57?> = WeakReference(mView)
    ...
    private class MyAdapter : RecyclerView.Adapter<ViewHolder>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            val view: View = LayoutInflater.from(parent.context)
                .inflate(R.layout.feed_item_057, parent, false)
            return ViewHolder(view)
        }
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            if(position %2 == 1) {
                holder.itemView.setBackgroundColor(Color.parseColor("#0065bb"))
            } else {
                holder.itemView.setBackgroundColor(Color.parseColor("#055ba4"))
            }
            ...
            CoroutineScope(Dispatchers.Main).launch {
                launch {
                    ...
                    if (data.start?.isNotEmpty() == true && data.end?.isNotEmpty() == true) {
                        ...
                        val dateFormat3: DateFormat = SimpleDateFormat("MMM dd, yyyy hh:mm a")
                        ...
                        var schedule: String = if (schedStart.get(Calendar.DAY_OF_YEAR) != todayDay)
                            SimpleDateFormat("MMM dd, yyyy hh:mm a").format(scheduleStartDate)
                        else
                            start.toString()
                        schedule = if (schedEnd.get(Calendar.DAY_OF_YEAR) != todayDay)
                            "  $schedule -\n" + SimpleDateFormat("MMM dd, yyyy hh:mm a").format(scheduleEndDate)
                        else
                            "  $schedule -\n$end"
    ...
    init {
        inflate(context, R.layout.feed_057, this)
        mHandler = ScrollHandler(this)
        mAdapter = MyAdapter()
        recyclerView = findViewById<View>(R.id.rv_feedss) as RecyclerView
